...
// ********************************************************************
//
// $Id: NTSTDetectorConstruction.cc,v 1.2 2007-10-26 09:51:29 gcosmo Exp $
...
NTSTDetectorConstruction::~NTSTDetectorConstruction()
{// G4cout<<"Deleting variables from NTSTDetCon "<<G4endl;
  delete _FileRead;
  delete DetectorMessenger;
   if (fEmFieldSetup) delete fEmFieldSetup;
   //G4cout<<"End deleting variables from NTSTDetCon "<<G4endl;
}
...
NTSTDetectorConstruction::Construct()
{
 ...
 // Construct the field creator - this will register the field it creates
  ...
  fEmFieldSetup->InitialiseAll();
 
  //------------------------------------------------------ materials

  G4double a;  // atomic mass
  G4double zMat;  // atomic number
  G4double density;
  G4String name;

  a = 39.95*g/mole;
  density = 1.782e-03*g/cm3;
  G4Material* Ar = new G4Material(name="ArgonGas", zMat=18., a, density);

  a = 26.98*g/mole;
  density = 2.7*g/cm3;
  // G4Material* Al = 
  new G4Material(name="Aluminum", zMat=13., a, density);

  a = 28.0855*g/mole;
  density = 2.33*g/cm3;
  G4Material* Si = new G4Material(name="Silicon", zMat=14., a, density);
