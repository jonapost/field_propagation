...
void NTSTFieldSetup::CreateStepperAndChordFinder()
{
  ...
  G4EquationOfMotion* pEquation = new G4Mag_UsualEqRhs(fMagneticField);
  G4VIntegrationDriver* pDriver = new BulirschStoerDriver(fMinStep, pEquation);
  fChordFinder = new G4RevisedChordFinder(pDriver);

  //fChordFinder = new G4RevisedChordFinder( fMagneticField, fMinStep,fStepper);
