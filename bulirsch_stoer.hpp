...
public:
    ...
    void set_max_dt(time_type max_dt)
    {
        m_max_dt = max_dt;
    }
    ...
    {
        m_error_checker.set_max_relative_error(eps_rel);
    }
    ...
    //method for QuickAdvance func
    //created by Dmitry Sorokin
    template< class System , class StateIn , class DerivIn , class StateOut >
    void quick_step( System system , const StateIn &in , const DerivIn &dxdt , time_type &t , StateOut &out , const time_type dt, double& err, state_type& yMid)
    {
        BOOST_USING_STD_MIN();
        BOOST_USING_STD_MAX();
        ...
        m_midpoint.set_steps(m_interval_sequence[0]);
        m_midpoint.do_step(system , in , dxdt , t , m_table[0].m_v , dt);
        extrapolate(1 , m_table , m_coeff , out);
        // get error estimate
