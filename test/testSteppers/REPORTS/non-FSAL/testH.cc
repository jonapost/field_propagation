...
void testIntegrator(/*G4MagInt_Driver *theDriver, G4FieldTrack &fTrack, G4int fout[]*/)
{
    ...
    for(int stepper_no=0; stepper_no<=max_no_steppers; stepper_no++)
    {
        ...
        for (int j=0; j<max_no_loops; j++) {
            ...
            func_evals = stages*theDriver->GetTotalNoStepperCalls() + theDriver->GetNoTotalSteps();
...
G4MagInt_Driver* SetupInt_Driver(G4int Stepper_no, G4FieldTrack theTrack, G4QuadrupoleMagField myField){
    //Create a field :
