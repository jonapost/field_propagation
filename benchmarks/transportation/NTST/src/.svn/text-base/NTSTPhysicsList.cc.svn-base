...
// ********************************************************************
//
// $Id: NTSTPhysicsList.cc,v 1.2 2007-10-26 09:51:29 gcosmo Exp $
...
NTSTPhysicsList::NTSTPhysicsList()
  : G4VUserPhysicsList(),
    ...
    onlyTransport(false),                       // NTST Physics by default
...
void NTSTPhysicsList::ConstructProcess()
{
  ...
  AddTransportation();
    ...
    G4cout << " +v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v+ " <<
      G4endl;
    if(onlyTransport){
      G4cout<<"!!!Only Transportation Process No Physics"<<
      G4endl;}; 
  if (!onlyTransport){
    ConstructEM();
    ConstructLeptHad();
...
{
  G4Exception("NTSTPhysicsList::AddBgsTransportation()", "NotImplemented",
              FatalException, "BgsTransportation is not enabled in this version -- due to migration to Touchable Handles");
#if 0
