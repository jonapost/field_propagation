...
G4FieldManager* SetupField(G4int type)
{
    ...
    switch ( type ) 
    {
        ...
        case 4: 
                {
                    fStepper = new StepperRK4_t(tEquation); break; 
                    pChordFinder = 
                        new TChordFinder
        ...
        G4int             type)
{
 ...
 /* G4EquationOfMotion* equationOfMotion = 
        static_cast<G4MagIntegratorStepper>( pMagFieldPropagator->GetChordFinder()->GetIntegrationDriver()->GetStepper())
        ->GetEquationOfMotion();
