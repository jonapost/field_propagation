...
void StatAccepTestEventAction::EndOfEventAction( const G4Event* evt ) {
  ...
  if ( G4VVisManager::GetConcreteInstance() ) {
    ...
    for ( G4int i=0; i<n_trajectories; i++ ) { 
      ...
      if ( trj->GetCharge() != 0. ) trj->DrawTrajectory(); // Draw only charged tracks
      // if ( trj->GetCharge() == 0. ) trj->DrawTrajectory(); // Draw only neutral tracks
      // if ( ( trj->GetCharge() == 0. ) && ( trj->GetParticleName() == "gamma" ) ) 
      //	trj->DrawTrajectory(); // Draw only gammas
      // if ( ( trj->GetCharge() == 0. ) && ( trj->GetParticleName() == "neutron" ) ) 
      //	trj->DrawTrajectory(); // Draw only neutrons
    }
  }
#endif
