...
G4bool BogackiShampine45::fPreparedConstants= false;
G4double BogackiShampine45::bi[12][7];
     ...
     fPreparedInterpolation(false)
{
    ...
    if( primary )
    {
       fAuxStepper = new BogackiShampine45(EqRhs, numberOfVariables, false);
    }
}
...
void
BogackiShampine45::GetLastDydx( G4double dyDxLast[] )
{
...
//    c8 = 3293.0/556956.0,
    ...
    for(i=0;i<numberOfVariables;i++)
    {
        yErr[i] = Step*(dc1*DyDx[i] + dc2*ak2[i] + dc3*ak3[i] + dc4*ak4[i] +
                        dc5*ak5[i] + dc6*ak6[i] + dc7*ak7[i] + dc8*ak9[i]) ;
...
G4double  BogackiShampine45::DistChord() const
{
    ...
    BogackiShampine45 *cThis= const_cast<BogackiShampine45 *>(this);
    // Use Interpolation, instead of auxiliary stepper to evaluate midpoint
    cThis-> SetupInterpolationHigh(); // ( fLastInitialVector, fLastDyDx, fLastStepLength ); // yInput, dydx, step );
    ...
    cThis->InterpolateHigh( tau, fMidVector );    
#endif
...
void BogackiShampine45::SetupInterpolationHigh()
    // ( const G4double *yInput, const G4double *dydx, const G4double Step)
{
    ...
    for(int i=0; i<numberOfVariables; i++){
        yTemp[i] = yIn[i] + Step*(a101*dydx[i] + a102*ak2[i] + a103*ak3[i] +
                                  ...
                                  a107*ak7[i] + a108*ak9[i] + a109*ak9[i] );
    }
    ...
    for(int i=0; i<numberOfVariables; i++){
        yTemp[i] = yIn[i] + Step*(a111*dydx[i] + a112*ak2[i] + a113*ak3[i] +
                                  ...
                                  a117*ak7[i] + a118*ak9[i] + a119*ak9[i] +
                                  a1110*ak10[i] );
    }
...
void BogackiShampine45::PrepareConstants()
{
    for(int i=1; i<= 11; i++)
        bi[i][1] = 0.0 ;
...
void BogackiShampine45::InterpolateHigh(G4double tau, G4double *yOut)
