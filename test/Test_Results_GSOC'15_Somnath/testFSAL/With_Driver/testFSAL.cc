/* Solely For the purpose of testing FSAL steppers */


/* Some Unused headers present */

#include "G4QuadrupoleMagField.hh"  //For using Quadrupole Magnetic field
#include "G4SystemOfUnits.hh"       //For Using the units mm etc.
//#include "G4ios.hh"
...
#include "G4PhysicalConstants.hh"       //For using the various Physical constants e.g. c
...
#define BogackiShampine45_stages 7
#define G4ClassicalRK4_stages 4
#define G4SimpleHeum_stages 2
#define G4ExplicitEuler_stages 1
#define VernerRK67_stages 10
#define VernerRK56_stages 9
#define VernerRK78_stages 13
...
FSALMagInt_Driver* SetupInt_Driver(G4int stepper_no,      //Create a new G4MagInt_Driver for us
                                   G4FieldTrack,
                                   G4QuadrupoleMagField);
void PrintResultOfStep(G4FieldTrack,        //Will print the (required) contents of G4FieldTrack
                       G4int stat_no);

void testIntegrator();          //The function for carrying out the test

G4FieldTrack CreateTrack();                 //For creating a brand new G4FieldTrack

G4QuadrupoleMagField SetupField();
...
void testIntegrator(/*G4MagInt_Driver *theDriver, G4FieldTrack &fTrack, G4int fout[]*/)
{
    ...
    FSALMagInt_Driver *theDriver = 0;
    G4QuadrupoleMagField myField = SetupField();
    G4FieldTrack fTrack = 0;
    G4double fout[11][20];//   [8][10];     //fout [max(stepper_no)] [no_of_loops]
    G4double epsOut[20];
    ...
    for(int stepper_no=0; stepper_no<=max_no_steppers; stepper_no++)
    {
        ...
        for (int j=0; j<max_no_loops; j++) {
            ...
            func_evals = stages*theDriver->GetTotalNoStepperCalls() + 1;
            fout[stepper_no][j] = func_evals;//log(func_evals)
            theEps *= factor;
...
FSALMagInt_Driver* SetupInt_Driver(G4int Stepper_no, G4FieldTrack theTrack, G4QuadrupoleMagField myField){
    //Create a field :
    
    //= new G4QuadrupoleMagField (G4ThreeVector(x_field, y_field, z_field));
    ...
    switch(Stepper_no){
            ...
            //            break;
        case 0: pStepper = new BogackiShampine45(fEquation);
            stages = BogackiShampine45_stages;
            stepper_name = "BogackiShampine45";
            break;
            //        case 0: pStepper = new G4ClassicalRK4(fEquation);
            ...
            //            break;
            //        case 6: pStepper = new VernerRK67(fEquation);
            //            stages = VernerRK67_stages - 1;
            //            stepper_name = "VernerRK67";
            //            break;
            //        case 5: pStepper = new VernerRK56(fEquation);
            //            stages = VernerRK56_stages - 1;
            //            stepper_name = "VernerRK56";
            //            break;
            //        case 7: pStepper = new VernerRK78(fEquation);
            //            stages = VernerRK78_stages - 1;
    ...
    //Setup the Driver
    FSALMagInt_Driver *theDriver = new FSALMagInt_Driver(hminimum,
                                                         pStepper,
                                                         numComponents,
                                                         statisticsVerbose);
...
G4QuadrupoleMagField SetupField(){
    ...
    G4QuadrupoleMagField myField(0.1*tesla/cm);
    return myField;
}
