                                 ...
                                 G4double hinitial )
{
  ...
  nstp=1;
    ...
    //Computing RHS for the first time
  ...
  do
  {
	...
	//From the 2nd time onwards, compute RHS only for NON-FSAL steppers
...
//------------------------------------------------------------
  ...
  for (iter=0; iter<max_trials ;iter++)
  {
    ...
    if( magvel_sq > 0.0 ) { 
       ...
       G4cerr << "** FSALMagIntegrationDriver: found case of zero momentum." 
                                  ...
                                  G4double&    dyerr )
{
  ...
  if(fVerboseLevel>2)
  {
    G4cout << "FSALMagIntDrv: Quick Advance" << G4endl;
