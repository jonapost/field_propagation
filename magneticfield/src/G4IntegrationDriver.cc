...
//
const G4double G4IntegrationDriver::max_stepping_increase = 5.0;
const G4double G4IntegrationDriver::max_stepping_decrease = 0.1;

//  The (default) maximum number of steps is Base
//  divided by the order of Stepper
//
const G4int  G4IntegrationDriver::fMaxStepBase = 100000;  // Was 5000, was 250
...
G4IntegrationDriver::G4IntegrationDriver(
    G4double hminimum,
...
G4bool G4IntegrationDriver::AccurateAdvance(
...
G4double G4IntegrationDriver::ShrinkStep(G4double error, G4double hstep)
{
    G4double htemp = GetSafety() * hstep * std::pow(error, GetPshrnk());
    return std::max(htemp, 0.1 * hstep);
}

G4double G4IntegrationDriver::GrowStep(G4double error, G4double hstep)
{
    G4double hnext;
    if (error > errcon) {
...
void G4IntegrationDriver::OneGoodStep(G4double y[],             // InOut
                                  const G4double dydx[],
...
//
G4bool  G4IntegrationDriver::QuickAdvance(
...
{
    G4Exception("G4IntegrationDriver::QuickAdvance()", "GeomField0001",
...
G4bool  G4IntegrationDriver::QuickAdvance(G4FieldTrack& track,
                                              const G4double dydx[],
                                              G4double hstep,
                                              G4double& dchord_step,
                                              G4double& dyerr)
{
    G4double yError[NCOMP], yIn[NCOMP], yOut[NCOMP];
...
void G4IntegrationDriver::GetDerivatives(const G4FieldTrack& track,
                                             G4double dydx[]) const
{
    G4double  y[NCOMP];
    track.DumpToArray(y);
...
void G4IntegrationDriver::SetVerboseLevel(G4int level)
{
    fVerboseLevel = level;
}

G4int G4IntegrationDriver::GetVerboseLevel() const
{
    return fVerboseLevel;
}

void G4IntegrationDriver::SetEquationOfMotion(G4EquationOfMotion *equation)
{
    pIntStepper->SetEquationOfMotion(equation);
}

G4EquationOfMotion* G4IntegrationDriver::GetEquationOfMotion()
{
...
G4double G4IntegrationDriver::ComputeNewStepSize(G4double  errMaxNorm,    // max error  (normalised)
...
//
G4double G4IntegrationDriver::ComputeNewStepSize_WithinLimits(
...
void G4IntegrationDriver::PrintStatisticsReport()
{
    G4int noPrecBig= 6;
    G4int oldPrec= G4cout.precision(noPrecBig);
    
    G4cout << "G4IntegrationDriver Statistics of steps undertaken. " << G4endl;
    G4cout << "G4IntegrationDriver: Number of Steps: "
    << " Total= " <<  fNoTotalSteps
    << " Bad= "   <<  fNoBadSteps 
    << " Small= " <<  fNoSmallSteps 
...
void G4IntegrationDriver::SetSmallestFraction(G4double newFraction)
{
