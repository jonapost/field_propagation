...
// ********************************************************************
//
// $Id: NTSTPhysicsListMessenger.cc,v 1.2 2007-10-26 09:51:29 gcosmo Exp $
...
:NTSTList(List)
{
  ...
  onlyTransportCmd = new G4UIcmdWithABool("/run/onlyTransport",this);
  onlyTransportCmd->SetGuidance( "True if onlyTransportation Process to be used" );
  onlyTransportCmd->SetGuidance( "No physics ! By default is false" );
  onlyTransportCmd->AvailableForStates(G4State_PreInit);  // New-state
...
void NTSTPhysicsListMessenger::SetNewValue(G4UIcommand* command,G4String s)
{
  ...
  } else if (command == onlyTransportCmd) {
    NTSTList->SetOnlyTransport( onlyTransportCmd->GetNewBoolValue(s) );
