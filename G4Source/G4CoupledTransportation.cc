                                             ...
                                             G4GPILSelection* selection )
{
  ...
  *selection = CandidateForSelection ;
  ...
  if( fieldExertsForce )
  {
     ...
     if( pChordFinder != nullptr)
     {
         // equationOfMotion = pChordFinder->GetEquationOfMotion();  // Use newest method
         equationOfMotion = 
                 pChordFinder->GetIntegrationDriver() // ->GetStepper()  // Available only for RK driver
                             ->GetEquationOfMotion();         
     }
     if(equationOfMotion != nullptr)
     {
...
G4CoupledTransportation::StartTracking(G4Track* aTrack)
{
  ...
  if( fGlobalFieldExists )
  {
     ...
     if( chordF != nullptr )  { chordF->ResetStepEstimate(); }
  }
